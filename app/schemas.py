...
class RerankIn(BaseModel):
    ...
    # [جديد — ميزة "التفكير"] لو True، الترتيب يعطي وزنًا أعلى لـautomata/
    # semantic (تفضيل شخصي فعلي) على حساب التشابه العام — يُستخدَم فقط
    # بعد تسلسل "أيقونة التفكير" حتى يكون تأثيرها ملموسًا فعلًا على
    # الريلز التالية، لا مجرد نص متحرك بلا أثر. راجع rerank_reels_by_embedding.
    sharpen: bool = False
...
class ThinkingOut(BaseModel):
    """[جديد] مخرج GET /reels/thinking — جمل عربية مُشتقّة فعليًا من حالة
    automata/taste_profile الحقيقية للمستخدم (لا نص ثابت/عشوائي). Flutter
    يعرضها بالتتابع، ثم يستدعي POST /reels/rerank مع sharpen=true."""
    sentences: list[str]
