...
class RerankIn(BaseModel):
    """مدخل POST /reels/rerank — قائمة reel_id كما رجّعتها Pexels لـ Flutter
    بالفعل (الباك اند لا يخزّن هذه القائمة، Flutter يرسلها في كل مرة)."""
    user_id: str
    reel_ids: list[str] = Field(default_factory=list)


class RerankOut(BaseModel):
    """مخرج POST /reels/rerank — نفس القائمة، مُعاد ترتيبها. عند عدم وجود
    UserEmbedding للمستخدم أو ReelEmbedding لأي من الريلز، تُرجَع القائمة
    بترتيبها الأصلي دون تغيير (تراجع آمن)."""
    reel_ids: list[str]
