...
async def record_interaction(
    ...
    # إضافي منفصل. [مُحدَّث — migration_0004]: rating هو الحقل الصحيح
    # دلاليًا لتقييم رضا المستخدم (final_rating نفسها، كما كانت تُستخدَم
    # فعليًا كهدف تدريب rating_head في neural.py). weight=final_rating
    # أُبقيت كما كانت (توافقًا مع أي قراءة سابقة لهذا العمود) — لكن
    # dataset.py (app/ml/dataset.py) يقرأ من rating حصريًا الآن، لا weight.
    log_interaction_v2(
        db,
        user_id=user_id,
        ...
        label=actual,
        weight=final_rating,
        rating=final_rating,  # [جديد — migration_0004] القيمة الصحيحة دلاليًا هنا فعلاً
        item_id=item_id,
        occasion=item.occasion_name,
