...
def log_interaction_v2(
    ...
    *,
    ...
    rating: Optional[float] = None,  # [جديد — migration_0004] رضا المستخدم (0..1)، مستقل عن label/weight
    ...
    """
    [مُحدَّث — migration_0004] أضيف معامل rating مستقل تمامًا عن label
    (قبول/رفض) وweight (وزن العينة). القيم الثلاث تُمرَّر كما هي من
    المتصل، لا تُعاد صياغتها هنا (نفس مبدأ label/weight الأصلي).
    ...
    try:
        ...
        row = InteractionV2(
            ...
            label=label,
            weight=weight,
            rating=rating,  # [جديد — migration_0004]
