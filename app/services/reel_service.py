...
async def rerank_reels_by_embedding(
    ...
    [جديد — ميزة "التفكير"] sharpen=True يستخدم أوزانًا بديلة
    (sharpen_*_weight في config.py) تفضّل automata/semantic (تفضيل شخصي
    فعلي) على حساب التشابه العام — يُستخدَم فقط في أول rerank بعد تسلسل
    "أيقونة التفكير" حتى يكون تأثيرها محسوسًا فعلًا على الفيد التالي، لا
    نص متحرك بلا أثر. راجع app/ml/automata/service.get_thinking_sequence.
    ...
    if sharpen:
        w_embedding = _AUTOMATA_CFG.sharpen_embedding_weight
        w_automata = _AUTOMATA_CFG.sharpen_automata_weight
        w_semantic = _AUTOMATA_CFG.sharpen_semantic_weight
    else:
        w_embedding = _AUTOMATA_CFG.hybrid_embedding_weight
        w_automata = _AUTOMATA_CFG.hybrid_automata_weight
        w_semantic = _AUTOMATA_CFG.hybrid_semantic_weight

    final_scores = (
        w_embedding * baseline_scores
        + w_automata * automata_component
