...
# يتبعان نفس الأسلوب).
#
# current_season وtemperature ليسا هنا عمدًا: سياقيان بحتان (لحظة
# الاستخدام/الطلب)، لا خاصية ذاتية للقطعة — يُمرَّران لـ extract_features
# من الاستدعاء نفسه وقت التدريب/الاستدلال، لا من هذا الجدول.
#
# ⚠️ dna_formal / dna_casual: nullable الآن عمدًا (لا يوجد بعد منطق حساب
# فعلي لهما وقت إنشاء القطعة). أي كود يستدعي extract_features() من بيانات
# هذا الجدول (مثل dataset.py) مسؤول عن تعويض None بقيمة افتراضية بنفسه
# (مثلاً 0.5) قبل التمرير — extract_features() نفسها لا تقبل None لهذين
# المعاملين (موقّعة كـ float غير Optional).
...
class Item(Base):
    ...
    occasion_name: Mapped[str | None] = mapped_column(String(64), nullable=True)  # مناسبة افتراضية للقطعة
    brand: Mapped[str | None] = mapped_column(String(128), nullable=True)
    is_layerable: Mapped[bool] = mapped_column(Boolean, default=False)

    # ⚠️ nullable عمدًا — انظر الملاحظة أعلاه
    dna_formal: Mapped[float | None] = mapped_column(Float, nullable=True)
    dna_casual: Mapped[float | None] = mapped_column(Float, nullable=True)
