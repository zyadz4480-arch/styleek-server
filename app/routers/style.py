...
POST /style/match               → جديد: مطابقة قطعة خارجية + الخزانة كاملة
                                   في نداء واحد (يعادل matchRemote في main.dart)
...
from app.ml import store, taste_profile
...
async def match(
    ...
    """يعادل matchRemote() في main.dart — نداء واحد يحمل القطعة الخارجية +
    خزانة المستخدم كاملة، ويرجع الدرجة والسبب وأفضل 6 قطع معًا. يستبدل
    نمط N نداء منفصل لكل قطعة خزانة (predictItemsBatchRemote) في مسار
    "هل تناسبني؟" تحديدًا."""
    return await service.match_item_with_wardrobe(
        db, user_id, payload.external_item, payload.wardrobe
    )
