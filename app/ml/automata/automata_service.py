...
# ─────────────────────────────────────────────────────────────────────────
# [جديد — ميزة "التفكير"] جمل عربية مُشتقّة من الحالة الفعلية، لا نص ثابت
# ─────────────────────────────────────────────────────────────────────────

_TAG_PHRASE_AR: dict[str, str] = {
    "formal": "الإطلالات الرسمية",
    "casual": "الإطلالات الكاجوال",
    "sporty": "الإطلالات الرياضية",
    "occasion_work": "إطلالات العمل",
    "occasion_university": "إطلالات الجامعة",
    "occasion_outing": "إطلالات الخرجات",
    "occasion_special": "إطلالات المناسبات الخاصة",
    "occasion_sport": "الإطلالات الرياضية",
}

# حد أدنى من الإشارات قبل التصريح بثقة عن نمط لوني — نفس عتبة
# get_inspiration (total_signals < 3) لعدم اختلاق استنتاج من عيّنة ضعيفة.
_MIN_SIGNALS_FOR_COLOR_CLAIM = 3


async def get_thinking_sequence(db: AsyncSession, user_id: str) -> list[str]:
    """[جديد] يبني تسلسل جمل عربية لـ"أيقونة التفكير" — كل جملة مشروطة
    بدليل فعلي من رسم automata أو avg_features لهذا المستخدم تحديدًا، لا
    نص Placeholder. لو الدليل غايب (مستخدم جديد كليًا)، تُستبدَل الجملة
    بأخرى صادقة تعكس الغياب نفسه بدل اختلاق استنتاج.

    ⚠️ هذه الدالة قراءة فقط — لا تُعدِّل أي حالة. التأثير "الملموس" الفعلي
    يأتي من استدعاء rerank لاحقًا بـsharpen=True من جهة العميل، لا من هنا.
    """
    sentences: list[str] = []

    # جملة 1: أقوى اهتمام فعلي (خلية automata الأعلى طاقة ولها وسم دلالي)
    try:
        graph, _ = await _load_graph(db, user_id)
        tagged_alive = [c for c in graph.alive_cells() if c.tag_votes]
        if tagged_alive:
            top_cell = max(tagged_alive, key=lambda c: c.energy)
            top_tag = max(top_cell.tag_votes, key=top_cell.tag_votes.get)
            phrase = _TAG_PHRASE_AR.get(top_tag, top_tag)
            sentences.append(
                f"ألاحظ أنك مهتم حاليًا بـ{phrase}، سأحاول اقتراح خيارات مشابهة وأكثر توافقًا مع ذوقك."
            )
    except Exception:
        logger.exception(f"[automata] فشل بناء جملة الاهتمام الأساسي لـuser_id={user_id}")

    # جملة 2: نمط لوني فعلي من avg_features — فقط لو عندنا إشارات كافية
    try:
        ...
        if total_signals >= _MIN_SIGNALS_FOR_COLOR_CLAIM:
            avg = list(profile.avg_features)
            saturation = avg[IDX_SATURATION]
            warm = avg[IDX_WARM_COLOR]
            if saturation < 0.35:
                color_phrase = "الألوان الهادئة"
            elif saturation >= 0.6:
                color_phrase = "الألوان الزاهية"
            else:
                color_phrase = None
            if color_phrase:
                if warm >= 0.65:
                    color_phrase += " والدافئة"
                elif warm <= 0.35:
                    color_phrase += " والباردة"
                sentences.append(
                    f"أرى أنك تتفاعل أكثر مع {color_phrase}، سأعطيها أولوية في الاقتراحات القادمة."
                )
    except Exception:
        logger.exception(f"[automata] فشل بناء جملة اللون لـuser_id={user_id}")

    # جملة افتتاحية بديلة صادقة لو ما فيه أي دليل بعد (مستخدم جديد كليًا)
    if not sentences:
        sentences.append("لسا أتعرف على ذوقك — كل تفاعل يساعدني أفهمك أكثر من الجولة اللي جاية.")

    # جملة الإغلاق — وصف للفعل القادم فعلًا (rerank بـsharpen=True)، لا ادّعاء بيانات
    sentences.append("سأبحث عن إطلالات تجمع بين تفضيلاتك الحالية وتنوع جديد قد يعجبك.")
