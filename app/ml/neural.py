...
class StyleNeuralNet:
    ...
    def predict_batch(self, X: np.ndarray) -> list[float]:
        """تنبؤ لعدة عناصر بنداء forward() واحد فقط (مش حلقة على predict_one) —
        هذا هو المكسب الفعلي من وجود /style/predict-batch بدل نداءات HTTP
        متعددة. يرجع final_score فقط (نفس القيمة اللي Flutter يستخدمها في
        predictBatchRemote عبر MLPrediction بكل الحقول = نفس الرقم)."""
        if not self.is_trained or len(X) == 0:
            return [0.5] * len(X)
        ...
        with torch.no_grad():
            x_t = torch.tensor(X, dtype=torch.float32)
            out = self.net(x_t)
            final = out["final"].numpy()

        return [float(v) for v in np.clip(final, 0.0, 1.0)]
