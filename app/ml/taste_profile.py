...
def _reel_partial_features(interaction: ReelInteraction) -> dict[int, float]:
    ...
    style_text = (interaction.outfit_style or "").lower()
    ...
    if interaction.dominant_color:
        rgb = _hex_to_rgb(interaction.dominant_color)
        if rgb is not None:
            r, g, b = rgb
...
def _reel_signal_weight(interaction: ReelInteraction) -> float:
    ...
    if signal == "watch":
        if not interaction.watch_seconds or not interaction.total_seconds:
            return 0.0
        ratio = interaction.watch_seconds / interaction.total_seconds
        if ratio >= 0.7:
            return 0.6 * ratio
        if ratio >= 0.3:
            return 0.2 * ratio
