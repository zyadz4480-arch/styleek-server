...
class InteractionV2(Base):
    ...
    # [جديد — migration_0004] رضا المستخدم (0..1)، مستقل عن label/weight
...
class ReelEmbedding(Base):
    ...
    # [جديد] يميّز مصدر الـembedding:
    #   - "cold_start": إسقاط خطي ثابت فوري (app/ml/cold_start.py)،
    #     يُنشأ تلقائيًا أول مرة يُطلب فيها embedding لريل لا يملك واحدًا
    #     بعد (عبر app/ml/embedding_repository.get_or_create_reel_embedding).
    #     دقته محدودة — إسقاط بذرة ثابتة، لا تعلّم فعلي.
    #   - "trained": ناتج فعلي من TwoTowerModel.encode_reel بعد تدريب
    #     حقيقي (app/ml/export_embeddings.py)، يستبدل قيمة cold_start
    #     تلقائيًا في upsert لاحق لنفس reel_id.
    # الفائدة: تتبّع أي صفوف لا تزال على قيمة ابتدائية غير مدرَّبة، وإمكانية
    # ترشيحها بمعزل عن الصفوف المدرَّبة فعليًا دون غموض.
    embedding_source: Mapped[str] = mapped_column(String(16), default="cold_start")
...
# ===========================
# [جديد — Graph Cellular Automata، دمج app/ml/automata]
# طبقة موازية اختيارية فوق UserEmbedding/ReelEmbedding (128 بُعد، نفس فضاء
# pgvector المستخدَم فعليًا في reel_rerank_service/reel_service) — لا تلمس
# UserStyleProfile.avg_features (22 بُعد، فضاء مختلف تمامًا). راجع
# integration_guide.md §3 وapp/ml/automata/service.py للتفاصيل الكاملة.
# ===========================

class AutomataCell(Base):
    __tablename__ = "automata_cells"

    cell_id: Mapped[str] = mapped_column(String(24), primary_key=True)
    user_id: Mapped[str] = mapped_column(String(128), index=True)

    embedding: Mapped[list[float]] = mapped_column(Vector(128))

    energy: Mapped[float] = mapped_column(Float)
    status: Mapped[str] = mapped_column(String(8))  # 'alive' | 'dormant' | 'dead'
    last_interaction_day: Mapped[int] = mapped_column(Integer)
    age: Mapped[int] = mapped_column(Integer, default=0)
    confidence: Mapped[float] = mapped_column(Float, default=0.5)
    generation: Mapped[int] = mapped_column(Integer, default=0)
    parent_ids: Mapped[list] = mapped_column(JSON, default=list)
    interaction_count: Mapped[int] = mapped_column(Integer, default=0)
    dormant_streak: Mapped[int] = mapped_column(Integer, default=0)

    # [جديد — وسم دلالي] {اسم_الوسم: قيمة تصويت تراكمية}، مثل
    # {"formal": 3.0, "occasion_work": 2.0}. إضافية بحتة، لا تدخل في أي
    # حساب طاقة. راجع app/ml/automata/cell.py Cell.tag_votes.
    ...
    __table_args__ = (
        Index("ix_automata_cells_user", "user_id"),
        Index("ix_automata_cells_user_status", "user_id", "status"),
    )


class AutomataEdge(Base):
    __tablename__ = "automata_edges"

    id: Mapped[int] = mapped_column(primary_key=True, autoincrement=True)
    user_id: Mapped[str] = mapped_column(String(128), index=True)
    cell_a: Mapped[str] = mapped_column(String(24))
    cell_b: Mapped[str] = mapped_column(String(24))
    strength: Mapped[float] = mapped_column(Float)
    last_reinforced_day: Mapped[int | None] = mapped_column(Integer, nullable=True)

    # [جديد] 'similarity' (تشابه embedding هندسي، السلوك الأصلي) أو
    # 'semantic' (تطابق وسم دلالي — formal/occasion، إلخ — راجع
    # automaton.py UserInterestGraph.semantic_edges). نفس زوج الخلايا
    # يمكن أن يملك صفًا من كل نوع في آن واحد، لذلك أُضيف لمفتاح التفرّد.
    edge_type: Mapped[str] = mapped_column(String(16), default="similarity")

    __table_args__ = (
        UniqueConstraint(
            "user_id", "cell_a", "cell_b", "edge_type",
            name="uq_automata_edges_user_pair_type",
        ),
        Index("ix_automata_edges_user", "user_id"),
    )


class AutomataGraphMeta(Base):
    __tablename__ = "automata_graph_meta"
    ...
    total_births: Mapped[int] = mapped_column(Integer, default=0)
    total_deaths: Mapped[int] = mapped_column(Integer, default=0)
    total_merges: Mapped[int] = mapped_column(Integer, default=0)
    last_nightly_run_at: Mapped[datetime | None] = mapped_column(
